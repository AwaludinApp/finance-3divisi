...
use App\Http\Requests\StoreTransaksiSecondRequest;
...
class SecondController extends Controller
{
    ...
    public function index()
    {
        ...
        //
        // $second = Second::where('is_deleted', 0)
        ...
        $akuns = Akun::where('is_deleted', 0)
            ...
            // return view('pages.transaksi.second.index', compact('second', 'akuns', 'title'));
            return view('pages.transaksi.second.index', compact('akuns', 'title'));
        }
     ...
     */
    public function store(StoreTransaksiSecondRequest $request)
    {
        //
        try {
    ...
    public function show(Second $second)
    {
        ...
        return response()->json([
            'result' => $second,
            'tanggal_transaksi' => date('d/m/Y', strtotime($second->tanggal_transaksi)),
            'nilai' => 'Rp. ' . number_format($second->nilai, 0, ',', '.')
        ]);
    }
     ...
     */
    public function edit(Second $bukuKeSecond)
    {
        //
    }
     ...
     */
    public function update(StoreTransaksiSecondRequest $request, Second $second)
    {
        //
        try {
            $data = $request->validated();
    ...
    {
        // $buku_besar = Second::where('is_deleted', 0)
        //     ->orderBy('tanggal_transaksi', 'ASC')
        //     ->where(function($q) {
        //         if (! in_array(Auth::id(), [1])) {
        //             $q->where('user_id', Auth::id());
        ...
        $totalRecords = Second::select('count(*) as allcount')
            ->where('is_deleted', 0)
            ->orderBy('tanggal_transaksi', 'ASC')
            // ->where(function($q) {
            //     if (! in_array(Auth::id(), [1])) {
        ...
        $totalRecordswithFilter = Second::with('akun')
            ->select('count(seconds.id) as allcount')
            ->leftJoin('akuns', 'akuns.id', '=', 'akun_id')
            ->where('seconds.is_deleted', 0)
            ->orderBy('tanggal_transaksi', 'ASC')
            // ->where(function($q) {
            //     if (! in_array(Auth::id(), [1])) {
            //         $q->where('user_id', Auth::id());
        ...
        $records = Second::with('akun')
            ->selectRaw('seconds.*, kode_akun, nama_akun')
            ->where('seconds.is_deleted', 0)
            ->leftJoin('akuns', 'akuns.id', '=', 'akun_id')
            ->orderBy('tanggal_transaksi', 'ASC')
            // ->where(function($q) {
        ...
        foreach($records as $record){
           ...
           $action = '
            ...
            <button class="btn btn-sm btn-danger" 
                ...
                data-id="'. $id .'"
                data-info="'. $kode_akun .'" 
                data-url="'. route('second.destroy', ['second' => $id]) .'">
